...
def compile_extension():
    cuda_source = Path("color_to_grayscale.cu").read_text()
    ...
    return grayscale_extension


def main():
    ext = compile_extension()

    input_image = read_image("input.jpg").permute(1, 2, 0).cuda()
    print(input_image.shape, input_image.dtype)

    output_image = ext.color_to_grayscale(input_image)  # type: ignore
    print(output_image.shape, output_image.dtype)
    write_jpeg(output_image.permute(2, 0, 1).cpu(), "./output.jpeg")
